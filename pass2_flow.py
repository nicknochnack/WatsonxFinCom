...
with TM1Service(
    ...
    data = tm1.cubes.cells.execute_view(
        cube_name="General Ledger", view_name="Pass 2_Input", private=False
    )
...
cost_centres = list(set(df.index.get_level_values("Cost Centre")))

cellset = {}
for cost_centre in cost_centres:
    df_cc = df.xs(cost_centre, level="Cost Centre")
    total_variance = (
        df_cc.xs("Amount", level="General Ledger Measure")
        .groupby("Sandboxes")
        .sum()["Values"]["Base"]
    )
    prompt = template(df_cc.to_dict()["Values"], total_variance)
    print(Fore.GREEN + prompt)
    response = llm.invoke(prompt)
    print(Fore.MAGENTA + response)
    print("\n\n")
    write_target = list(df_cc.iloc[0].name)[:-1]
    write_target.insert(4, cost_centre)
